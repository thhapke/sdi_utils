...
try:
    ...
    class api:
        ...
        class config:
            ...
            tags = {'sdi_utils':''}
            version = "0.0.1"
            operator_description = "csv stream to table"
            operator_description_long = "Converts csv stream to table"
            add_readme = dict()
            debug_mode = True
...
def process(msg):
    ...
    att_dict["table"] = {"name": msg.attributes["storage.filename"] , "version": 1, "columns": list()}
    ...
    if msg.attributes["storage.endOfSequence"] or api.config.collect == False:
        ...
        msg = api.Message(attributes=att_dict, body=result_table)
